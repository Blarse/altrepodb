...
@Timing.timeit(NAME)
def insert_assignment_name(conn, logger, assignment_name=None, uuid=None, tag=None, assignment_date=None, complete=0):
...
@Timing.timeit(NAME)
def insert_pkgset_name(conn, logger, name, uuid, puuid, ruuid, depth, tag, date, complete, kv_args):
    if date is None:
...
def insert_pkgset(conn, logger, uuid, pkghash):
    ...
    logger.debug('insert package set uuid: {0}, pkg_hash: {1}'.format(uuid, len(pkghash)))
...
class Worker(threading.Thread):
    def __init__(self, connection, logger, pkg_cache, src_repo_cache, pkg_repo_cache, packages, aname,
            ...
            try:
                ...
                with self.lock:
                    if self.is_src:
                        ...
                        if self.src_repo_cache[pkg_filename]['blake2b'] in (b'', None):
                            self.logger.debug(f"calculate BLAKE2b for {pkg_filename} file")
                ...
                if self.repair is not None:
                    if pkghash is not None:
                        repair_package(self.connection, self.logger, header, self.repair, **kw)
                    continue
                if pkghash is None:
                    pkghash = insert_package(self.connection, self.logger, header, package, **kw)
...
def worker_pool(pkg_cache, src_repo_cache, pkg_repo_cache, packages_list, pkgset, display, is_src, args, logger):
    workers = []
    connections = []
    ...
    for i in range(args.workers):
        ...
        worker = Worker(conn, logger, pkg_cache, src_repo_cache, pkg_repo_cache, packages, pkgset, display, args.repair, is_src)
        worker.start()
            ...
            if len(v) == 3:
                ...
                if kk in repo_cache.keys():
                    if v[1] != 0:
                        ...
                        cnt2 += 1
    logger.debug(f"Requested {len(result)} package hashes from database. "
...
def read_repo_structure(repo_name, repo_path, logger):
                ...
                if arch == 'src':
                    ...
                    for c in contents:
                        ...
                        if pkg_name not in repo['src_hashes']:
                            logger.info(f"{pkg_name}'s MD5 not found. Calculating it from file")
        ...
        for k, v in repo['src_hashes'].items():
            if v["sha256"] is None:
                ...
                if file_.is_file():
                    ...
                    logger.error(f"Can't find file to calculate SHA256 for {file_.name} from {file_.parent}")
                    raise RuntimeError("File not found")
            if v["blake2b"] in (b'', None) and repo["use_blake2b"]:
                ...
                if file_.is_file():
                    ...
                    logger.error(f"Can't find file to calculate blake2b for {file_.name} from {file_.parent}")
                    raise RuntimeError("File not found")
        # for binary files
    ...
    logger.debug(f"Found {len(repo['comp']['comps'])} components for {len(repo['arch']['archs'])} architectures")
...
def load(args, logger):
    ...
    if iso:
        ...
        if check_repo_date_name_in_db(conn, args.pkgset, args.date.date()):
            if not args.force:
                logger.error(f"Repository with name '{args.pkgset}' and"
        ...
        if args.repair is None:
            ...
            worker_pool(cache, repo['src_hashes'], repo['pkg_hashes'], packages_list, pkgset, display, True, args, logger)
                    ...
                    if rpm_file.suffix == '.rpm':
                        ...
                        if repo['pkg_hashes'][rpm_file.name]['sha1'] is None:
                            ...
                            if not pkgh:
                                ...
                                raise ValueError(msg)
                            pkgset_cached.add(pkgh)
                logger.info(f"Checked {pkg_count} RPM packages. "
                         f"{len(packages_list)} packages for load. "
                         f"Time elapsed {(time.time() - ts):.3f} sec.")
                # load '.rpm' packages
                worker_pool(cache, repo['src_hashes'], repo['pkg_hashes'], packages_list, pkgset, display, False, args, logger)
                # build pkgset for PackageSet record
                pkgset.update(pkgset_cached)
