...
DATETIME_FMTs = (
    ...
    (
        re.compile(r"[F-W][a-u]{2} [A-S][a-y]{2} +\d+ \d{2}:\d{2}:\d{2} \d{4}"),
        "%a %b %d %H:%M:%S %Y",
    ),
    (
        re.compile(r"[F-W][a-u]{2}, \d+ [A-S][a-y]{2} \d{4} \d{2}:\d{2}:\d{2} .+"),
        "%a, %d %b %Y %H:%M:%S %Z",
    ),
...
def parse(soup):
    ...
    tables = (
        [x for x in soup.find_all("table") if x.find(string=RE_COMMONHEAD)]
        if not pres
        else ()
    )
    heads = []
        ...
        for element in pre.hr.next_siblings if pre.hr else pre.children:
            if element.name == "a":
                if not element.string or not element.string.strip():
                    ...
                    if file_name:
                        listing.append(
                            FileEntry(file_name, file_mod, file_size, file_desc)
                        )
                    file_name = aherf2filename(element["href"])
                    file_mod = file_size = file_desc = None
                elif (
                    element.string in ("Parent Directory", "..", "../")
                    or element["href"][0] not in "?/"
                ):
                    started = True
            ...
            if started:
                ...
                for td in tr.find_all("td"):
                    ...
                    if td.get("colspan"):
                        ...
                        file_desc = (
                            file_desc
                            or "".join(map(str, td.children)).strip(
                                " \t\n\r\x0b\x0c\xa0"
                            )
                            or None
                        )
